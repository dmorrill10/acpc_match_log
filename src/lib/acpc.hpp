...
// for this.
template <typename List> double logsumexp(List nums) {
...
ResultType reduce(const SrcArrayType src, ReduceFn reduceFn,
                  ResultType result) {
  for (auto elem : src) {
...
constexpr uint32_t handNum(const State &state) { return state.handId + 1; }
...
bool allOthersFolded(const State &state, const size_t pos,
...
public:
  static int32_t bigBlind(const Game *game) {
    return (reduce(game->blind,
                   game->numPlayers, [](int32_t maxSoFar, int32_t blind) {
                     return blind > maxSoFar ? blind : maxSoFar;
                   }, 0));
  }
  GameDef(const GameDef &toCopy)
      : game_(nullptr), bigBlind_(bigBlind(toCopy.game_)) {
  ...
  GameDef(const std::string &gameDefPath)
      : game_(gameDef(gameDefPath)), bigBlind_(bigBlind(game_)){};
...
std::vector<std::string> players(const std::string &stateString,
  ...
  std::string playersString = stateString.substr(
      charsRead + 1 + stateString.substr(charsRead + 1).find(":") + 1);
 ...
 */
void replay(const MatchState &view, const GameDef &gameDef,
  ...
  memcpy(ms.state.holeCards, view.state.holeCards,
         sizeof(view.state.holeCards));

  for (uint8_t round = 0; round <= view.state.round; ++round) {
    for (uint8_t actionIndex = 0; actionIndex < view.state.numActions[round];
         ++actionIndex) {
      const Action &action = view.state.action[round][actionIndex];
      if (doOnState(ms, action)) {
        return;
      }
...
struct DealerConnection {
  DealerConnection(uint16_t port, const std::string &host = "localhost")
      : port_(port), host_(), toServer_(nullptr), fromServer_(nullptr) {
  ...
  void connect() {
    ...
    if (fprintf(toServer_, "VERSION:%" PRIu32 ".%" PRIu32 ".%" PRIu32 "\n",
...
class Configuration {
public:
  Configuration(const std::string &gameDefPath, uint16_t port,
                const std::string &host = "localhost")
      : gameDef_(gameDefPath), dealer_(port, host) {
    dealer_.connect();
  }
  Configuration(const GameDef &gameDef, uint16_t port,
                const std::string &host = "localhost")
  ...
  void sendAction(Action actionToSend, char *line, int len) {
    ...
    int r = printAction(gameDef_.game_, &actionToSend, MAX_LINE_LEN - len - 2,
                        &line[len]);
  ...
  void
  forEveryMatchState(std::function<Action(const MatchState &)> generateAction,
  ...
  bool mustAct() const {
    return (!handFinished() &&
            currentPlayer(gameDef_.game_, &state_.state) ==
                state_.viewingPlayer);
  }
...
constexpr double complementaryProb(double p) { return 1.0 - p; }

constexpr double sampledImmediateCfr(double infoSetW, double probTakingAction,
                                     bool actionIsPartOfSeq) {
  return actionIsPartOfSeq ? infoSetW * complementaryProb(probTakingAction)
                           : -infoSetW * probTakingAction;
}
...
bool isBeginningOfHand(const State &state) { return state.numActions[0] < 1; }
...
bool yetToActThisRound(const State &state, uint8_t viewingPlayer, uint round) {
  ...
  for (int actionIndex = 0; actionIndex < state.numActions[round];
       ++actionIndex) {
...
bool yetToActThisRound(const MatchState &view, uint8_t viewingPlayer,
                       uint round) {
...
bool actionIsAtEndOfSequence(const Action &action, const MatchState &view) {
  const auto &numActionsInCurrentRound =
      view.state.numActions[view.state.round];
  if (numActionsInCurrentRound <= 0) {
    ...
    return actionsEqual(
        view.state.action[prevRound][view.state.numActions[prevRound] - 1],
        action);
  }
  return actionsEqual(
      view.state.action[view.state.round][numActionsInCurrentRound - 1],
      action);
}
...
double regretMatching(const Container regrets, size_t actionIndex) {
  ...
  const double sumOfPosRegrets =
      reduce(regrets, [](double posRegret, double r) {
        return r > 0.0 ? posRegret + r : posRegret;
      }, 0.0);

  return (sumOfPosRegrets > 0.0
...
double normalize(const Container values, size_t subjectIndex) {
  ...
  const double sumOfValues =
      reduce(values, [](double s, double val) { return s + val; }, 0.0);

  return (sumOfValues > 0.0 ? (values[subjectIndex] / sumOfValues)
                            : (1.0 / values.size()));
}

template <class Count>
double optimisticAveragingWeight(uint32_t handNum, Count infoSetCount,
                                 double probIPlayToCurrentInfoSet) {
  return (handNum - infoSetCount) * probIPlayToCurrentInfoSet;
}

template <size_t numCards> class Deck {
public:
  ...
  constexpr size_t numHiddenCards() const { return (~cardsRevealed_).count(); }

  template <typename CardIndex> std::vector<CardIndex> hiddenCards() const {
...
namespace Dealer {
int startMatch(const std::string &matchName, const GameDef &gameDef,
               const std::vector<std::string> &players,
               const std::vector<int> &listenSocket,
               const std::string &workingDirectory, uint32_t numHands = 3000,
               ...
               bool useLogFile = 0, bool useTransactionFile = 0) {
  ...
  for (int i = 0; i < game->numPlayers; ++i) {
    seatName[i] =
        static_cast<char *>(calloc(players[i].size(), sizeof(*seatName[i])));
    memcpy(seatName[i], players[i].data(),
           sizeof(*seatName[i]) * players[i].size());
  }
  ...
  if (useTransactionFile) {
    const auto transactionFileName =
        workingDirectory + "/" + matchName + ".tlog";
    memcpy(name, transactionFileName.data(),
           sizeof(*name) * transactionFileName.size());
